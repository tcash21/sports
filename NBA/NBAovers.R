...
y <- y[seq(1,dim(y)[1], by=2),]
y$away_team<-as.factor(y$away_team)
y$home_team <- as.factor(y$home_team)
y$positive<-as.factor(y$positive)

r <- randomForest(over ~ numChanges + line_change + positive, data=y)
